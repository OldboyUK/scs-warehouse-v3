...
const HOURS_WORKED_STATUS_URL = `${window.location.origin}/.netlify/functions/getHoursWorkedApprovals`;
...
function showChoice() {
  ...
  app.innerHTML = `
    ...
    <div class="dash-grid">
      ...
      <button type="button" class="dash-card" onclick="showHoursWorkedStatus()">
        <span class="dash-icon">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 2v4"/><path d="M16 2v4"/><rect width="18" height="18" x="3" y="4" rx="2"/><path d="M3 10h18"/><path d="M8 14h.01"/><path d="M12 14h.01"/><path d="M16 14h.01"/><path d="M8 18h.01"/><path d="M12 18h.01"/><path d="M16 18h.01"/></svg>
        </span>
        <span class="dash-card-title">View Hours Worked</span>
        <span class="dash-card-desc">See approval status for your recorded hours</span>
      </button>
    </div>
  `;
}
...
function requireSignedIn() {
  ...
  app.innerHTML = `
    <div class="hr-form-card">
      <p class="status text-error">You must be signed in to view approval status.</p>
      <div class="actions">
...
function showOvertimeStatus() {
  const username = requireSignedIn();
  if (!username) return;
  ...
  fetch(OVERTIME_STATUS_URL, {
    ...
    .then(data => {
      ...
      renderApprovalTable({
        ...
        welcomeLabel: 'Overtime approvals',
        emptyMessage: 'No overtime records found for your account.',
        hoursHeading: 'Overtime Hours',
        hoursKey: 'overtimeHours',
        summaryTitle: 'Overtime Summary'
      });
    })
    .catch(err => {
      console.error(err);
...
function showHoursWorkedStatus() {
  const username = requireSignedIn();
  if (!username) return;

  app.innerHTML = `
    <p class="hr-welcome">Hours worked approvals for <span>${escapeHTML(username)}</span></p>
    <p class="status">Loading your hours worked records…</p>
  `;

  const body = new URLSearchParams();
  body.append('username', username);

  fetch(HOURS_WORKED_STATUS_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body
  })
    ...
    .then(data => {
      if (data.result !== 'success') {
        throw new Error(data.message || 'Failed to load hours worked records.');
      }
      renderApprovalTable({
        username,
        rows: Array.isArray(data.rows) ? data.rows : [],
        welcomeLabel: 'Hours worked approvals',
        emptyMessage: 'No hours worked records found for your account.',
        hoursHeading: 'Hours Worked',
        hoursKey: 'hoursWorked',
        summaryTitle: 'Hours Worked Summary'
      });
    })
    .catch(err => {
      console.error(err);
      app.innerHTML = `
        <div class="hr-form-card">
          <p class="status text-error">${escapeHTML(err.message || 'Failed to load hours worked records.')}</p>
          <div class="actions">
            <button type="button" class="btn btn-ghost" onclick="showChoice()">Back</button>
            <button type="button" class="btn btn-primary" onclick="showHoursWorkedStatus()">Try again</button>
          </div>
        </div>
      `;
    });
...
function summariseHours(rows, hoursKey) {
  let approvedMinutes = 0;
  let awaitingMinutes = 0;

  for (const row of rows) {
    const minutes = parseHoursToMinutes(row[hoursKey]);
...
function renderApprovalTable({
  ...
  if (!rows.length) {
    app.innerHTML = `
      <p class="hr-welcome">${escapeHTML(welcomeLabel)} for <span>${escapeHTML(username)}</span></p>
      <div class="hr-form-card">
        <p class="hr-intro">${escapeHTML(emptyMessage)}</p>
        <div class="actions">
          <button type="button" class="btn btn-ghost" onclick="showChoice()">Back</button>
        </div>
      </div>
  ...
  const summary = summariseHours(rows, hoursKey);
  ...
  const bodyRows = rows.map(row => {
    ...
    return `
      <tr>
        <td data-label="${escapeHTML(hoursHeading)}">${escapeHTML(row[hoursKey] || '-')}</td>
  ...
  app.innerHTML = `
    ...
    <div class="hr-form-card hr-overtime-summary">
      <h2 class="hr-form-section-title">${escapeHTML(summaryTitle)}</h2>
...
window.showHoursWorkedStatus = showHoursWorkedStatus;
